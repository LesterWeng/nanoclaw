...
console.log('=== NanoClaw Start Test ===\n');

// Test 1: Check if script can be compiled
try {
  console.log('1. Testing TypeScript compilation...');
  execSync('npx tsc scripts/start.ts --outDir test-dist --target es2020 --moduleResolution node --esModuleInterop --lib es2020 --noImplicitAny --noImplicitReturns --noUnusedLocals --strict', {
...
try {
  ...
  const result = execSync('npx tsx scripts/start.ts', {
    cwd: path.resolve(__dirname, '..'),
    stdio: 'pipe',
    timeout: 5000
  });
  ...
  // Expected to fail since service might not be configured, but should parse correctly
  if (error.message.includes('Service not configured') ||
      error.message.includes('NanoClaw start script only supports')) {
...
try {
  ...
  if (!manifestContent.includes('skill:') || !manifestContent.includes('start')) {
