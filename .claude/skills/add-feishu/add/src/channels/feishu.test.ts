...
describe('FeishuChannel', () => {
  ...
  describe('ownsJid', () => {
    it('should own feishu JIDs', () => {
      expect(channel.ownsJid('oc_xxxxx@feishu')).toBe(true);
    });
    ...
    it('should not own non-feishu JIDs', () => {
      ...
      expect(channel.ownsJid('oc_xxxxx')).toBe(false);
    });
  });
  ...
  describe('setTyping', () => {
    it('should be callable (no-op for Feishu)', async () => {
      await expect(channel.setTyping('oc_xxxxx@feishu', true)).resolves.toBeUndefined();
    });
  });
